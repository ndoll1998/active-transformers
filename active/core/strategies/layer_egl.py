...
class AbstractLayerEgl(AbstractStrategy):
    ...
    def process(self, batch:Any) -> torch.FloatTensor:
        ...
        with GoodfellowGradientNorm(self.model) as gradnorm:
            ...
            with torch.no_grad():
                # average out gradients to prevent strategy from preferring
...
class LayerEglByTopK(AbstractLayerEgl):
    ...
    def _get_hallucinated_labels(
        ...
        if (logits.ndim == 3) and (self.k == 1):
            p, labels = torch.topk(probs, k=1, dim=-1)
            return p.prod(dim=-2), labels
...
class LayerEglBySampling(AbstractLayerEgl):
    """ Implements Maximum Expected Gradient Length Sampling Strategy
        ...
        Args:
            ...
            k (int):
                maximum number of targets to consider for
                expectation approximation. Defaults to 8.
    """
    ...
    def _get_hallucinated_labels(
        ...
        probs = torch.full((logits.size(0), self.k), fill_value=1.0/self.k, device=labels.device)
        return probs, labels
