...
def attach_metrics(engine):
    ...
    R = Recall(output_transform=type(trainer).get_logits_and_labels, average=True)
    P = Precision(output_transform=type(trainer).get_logits_and_labels, average=True)
    F = Fbeta(beta=1.0, output_transform=type(trainer).get_logits_and_labels, average=True)
    # attach metrics
    L.attach(engine, 'L')
    R.attach(engine, 'R')
    P.attach(engine, 'P')
...
if __name__ == '__main__':
    ...
    parser.add_argument("--pretrained-ckpt", type=str, default="distillbert-base-uncased", help="The pretrained model checkpoint")
    ...
    parser.add_argument("--batch-size", type=int, default=64, help="Batch size to use during training and evaluation")
    ...
    filter_fn = lambda e: len(e['tokens']) > 0
    train_dataset = train_dataset.filter(filter_fn, batched=False, load_from_cache_file=args.use_cache, desc="Filtering")
    val_dataset = val_dataset.filter(filter_fn, batched=False, load_from_cache_file=args.use_cache, desc="Filtering")
    test_dataset = test_dataset.filter(filter_fn, batched=False, load_from_cache_file=args.use_cache, desc="Filtering")
    # preprocess
    train_dataset = train_dataset.map(processor, batched=False, load_from_cache_file=args.use_cache, desc="Processing")
    val_dataset = val_dataset.map(processor, batched=False, load_from_cache_file=args.use_cache, desc="Processing")
    test_dataset = test_dataset.map(processor, batched=False, load_from_cache_file=args.use_cache, desc="Processing")
    # and set formats
    ...
    model = DistilBertForTokenClassification.from_pretrained('distilbert-base-uncased', num_labels=processor.num_labels)
